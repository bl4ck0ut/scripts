...
//const char* ssid     = "wookie newest";      // SSID of local network---------------------USER Input here
//const char* password = "anitacharley090305";   // Password on network -----------------------------------------------------

#ifndef STASSID
#define STASSID "wookie newest"
#define STAPSK "anitacharley090305"
#endif
...
//  WiFi.begin(ssid, password);
 ...
 if (isnan(t) || isnan(h)) {
  ...
  else if (t >= DHT_max_temp) {
    Serial.println("Temp too High");
      const char host[ ]        = "maker.ifttt.com";          // maker channel of IFTTT
  const char trigger[ ]     = "temp";                   //name of the trigger you would like to send to IFTTT --------------------
  const char APIKey[ ]      = "fARIv4GiByZLJJiTBRXpW7pHyK6q9VOtGP4zwle_pBr";      //Your maker key for Webhooks on IFTTT-----------
  ...
  //ESP.deepSleep(7200e6); //30 for 30 seconds 7200 for 2 hours 86400 for 1 day
  ESP.deepSleep(30e6); //30 for 30 seconds 7200 for 2 hours 86400 for 1 day
  }
  else {
    Serial.print("Things are ok going to sleep");
    Serial.print("Humidity: "); 
    Serial.print(h);
    Serial.print(" %\t");
